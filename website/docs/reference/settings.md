...
| function | returns |
| --- | --- |
| `mssql_ducklake_version()` | the extension's release version, `0.1.1` in this one — the same idea as the mssql extension's `mssql_version()`; a build between releases answers the coming version with `-dev`. `duckdb_extensions()` shows the build's git hash instead. |
